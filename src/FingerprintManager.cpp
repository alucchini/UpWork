...
bool FingerprintManager::connect() {
    ...
    finger.LEDcontrol(FINGERPRINT_LED_FLASHING, 25, FINGERPRINT_LED_RED, 0); // sensor connected signal
...
Match FingerprintManager::scanFingerprint() {
  ...
  while (doAnotherScan)
  {
    ...
    while (doImaging)
    {
      ...
      while (match.returnCode != FINGERPRINT_OK)
      {
        match.returnCode = finger.getImage();
      }
...
void FingerprintManager::loadFingerListFromPrefs() {
  ...
  if (counter != finger.templateCount)
    Serial.print("Warning: Fingerprint count mismatch! ");
    Serial.print(finger.templateCount);
    Serial.print(" fingerprints stored on sensor, but we are aware of ");
...
NewFinger FingerprintManager::enrollFinger(int id, String name) {
  ...
  Serial.print("Enrollment for id #");
  Serial.print(id);
  Serial.println(" started. We need to scan your finger 5 times until enrollment is completed.");
  ...
  for (int nTimes=1; nTimes<=5; nTimes++)
  {
      Serial.print("Take #");
      Serial.print(nTimes);
      Serial.println(" (place your finger on the sensor until led ring stops flashing, then remove it).");
...
void FingerprintManager::deleteFinger(int id) {
  ...
  if ((id > 0) && (id <= 200)) {
    ...
    if (result != FINGERPRINT_OK) {
      Serial.print("Delete of finger template #");
      Serial.print(id);
      Serial.print(" from sensor failed with code ");
...
void FingerprintManager::setIgnoreTouchRing(bool state) {
  if (ignoreTouchRing != state) {
    ...
    if (state == true)
      Serial.println("IgnoreTouchRing is now 'on'");
    else
      Serial.println("IgnoreTouchRing is now 'off'");
  }
}
...
void FingerprintManager::setLedRingReady() {
  if (!ignoreTouchRing)
    ...
    finger.LEDcontrol(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_RED); // just an indicator for me to see if touch ring is active or not
}
...
String FingerprintManager::generateNewPairingCode() {
    ...
    for (byte i=0; i < 5; i++) // use only the first 5 bytes of hash
    {
